...
def raw_df_convert(raw_df,lines_to_drop=None,standardise=True,labels = 'metric'):
    # drop unwanted rows from dataset
    df = raw_df.drop(lines_to_drop, axis=0)
    df.dropna(inplace = True)
    df = df.reset_index(drop=True)
    cols = df[labels].values
...
def get_onset_offset_index(df,spike=0,min_rate=1.5):
	...
	if len(peaks[0])>1:
		if (peaks[0][1] - peaks[0][0])<200:
			offset_index =  np.where(df['Voltage'].loc[peak:]==min(df['Voltage'].loc[peak:peaks[0][1]+ df.index[0]]))[0][0] + peak
		else:
			try:
...
def access_resistance(df):
	Ra = df['Access_Resistance'].unique()[0]
	return Ra
...
def tau(df):
	'''From Isokawa 1996'''
	from math import e
	windows = [df[10000:13000],df[30000:33000],df[50000:53000],df[70000:73000],df[90000:93000]]

	def iso_tau(df_tau):
		v1 = df_tau[0]
		v2 = np.mean(df_tau[-2000:])
		dv = v2-v1
		v_tau = v2 - (1/e)*dv 
		t_tau = np.where(df_tau<=v_tau)[0][0]#

		return t_tau*0.1

	taus = []
	for i,series in enumerate(windows):
		dft = series
		t = iso_tau(dft['Voltage'].values)
		taus.append(t)
...
def Refractory_period(df):
	...
	if len(peaks)>1:
		...
		if (peaks[1] - peaks[0])<=300:
			...
			node = find_peaks(rb_df['Voltage'].loc[off:off+500],
				distance=50,
				height=[min(rb_df['Voltage'].loc[off:off+500]+0.5),0.0],
				prominence=0.75,
				# threshold=sAHP-100,
				width=20)
	...
	########################################################################################################################################                
		...
		node = find_peaks(rb_df['Voltage'].loc[off:off+1000],
			distance=100,
			height=[min(rb_df['Voltage'].loc[off:off+1000])+0.5,0.0],
			prominence=0.5,
			# threshold=sAHP-100,
			width=22.5)
		if len(node[0])>0:
			ADP_idx = node[0][0] + off
			if (rb_df.index[-1]-ADP_idx)>100:
				fAHP_idx = np.where(rb_df['Voltage'].loc[peaks[0]:ADP_idx]==min(rb_df['Voltage'].loc[peaks[0]:ADP_idx]))[0][0] + peaks[0]
				fAHP_min = min(rb_df['Voltage'].loc[peaks[0]:ADP_idx])
				fAHP = fAHP_min - rb_df['Voltage'].loc[on]
				ADP = rb_df['Voltage'].loc[ADP_idx] - fAHP_min
...
def AP23_drop(df):
	...
	APs = find_peaks(AP_range['Voltage'],height=0.0)[1]
	if len(APs['peak_heights']) > 2:
		AP23_drop = APs['peak_heights'][2] - APs['peak_heights'][1]
	else:
		AP23_drop = 0.0
	return AP23_drop
	...
	if stim=='ramp':
		if len(peaks)>0:
			try:
				...
				threshold = ramp['Voltage'].loc[on]
	if stim=='rheo':
		on,off = get_onset_offset_index(rb,min_rate=1.0)
...
def rb2x_ISI_1_3(df):
	...
	for i,dft in enumerate(rb210):
		...
		ISIs = []
		if len(peaks)<2:
				ISI = (dft['time'].iloc[-1] - dft['time'][peaks[0]])*1000
				ISIs.append(ISI)
		else:
...
def Avg_spike_rate(df):
	...
	for i,dft in enumerate(rb210):
		...
		peaks+= dft.index[0]
		if len(peaks<2):
			sps = len(peaks)/(dft['time'].iloc[-1] - dft['time'][peaks[0]]) 
		else:
			sps = len(peaks)/(dft['time'][peaks[-1]]- dft['time'][peaks[0]])

	num_spikes.append(sps)

	return np.mean(num_spikes)

def Max_spike_rate(df):
	rb4x = rheobasex4_df(df)
	peaks = find_peaks(rb4x['Voltage'],height=5.0,
										width=[5,100],
                     					prominence=1.0)[0]
	peaks+= rb4x.index[0]
	if len(peaks)<=2:
			max_rate = len(peaks)/(rb4x['time'].iloc[-1] - rb4x['time'][peaks[0]]) 
	else:
			max_rate = len(peaks)/(rb4x['time'][peaks[-1]]- rb4x['time'][peaks[0]])
...
def make_param_df(df):
	...
	Parameter_dict ={'Recording_Date':get_date(df),
                 ...
                 'Delay_to_Spike_2x':delay_to_spike_2x(df),
                 ...
                 'Recorded_Access_Resistance':access_resistance(df)
}

	df_params = pd.DataFrame.from_dict(Parameter_dict,orient='index',columns = df.mouseID.unique()+'_'+df.Cell.unique())
