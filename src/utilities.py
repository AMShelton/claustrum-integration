...
def GS_to_df(certificate,sheet = None,tab=0):
	...
	#authorization
	gc = pygsheets.authorize(service_file=certificate) #accesses the sheet 
	#open GS
	sh = gc.open(sheet)
	wks = sh[tab]
	#get data
...
''' 
    response_windows = [df.iloc[stim-before:stim+after].values for i,stim in enumerate(stims[start:stop:step])]

    return response_windows

def stim_onset(data,distance=None,num_std=None,width=300):
	stim_frames = find_peaks(data,height=(np.mean(data)+np.std(data)*num_std),width=width)[0]
	stims = [stim_frames[0]]
...
def find_pulse(data,mean_intvl=5000,std=5,pulse_width=70):
